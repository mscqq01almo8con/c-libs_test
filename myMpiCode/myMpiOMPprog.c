...
/*	Editable variable definitions are the indented ones.		*/
				...
				//	e.g. for 16 processes we set (CARTESIAN_X=4)*(CARTESIAN_Y=4) = 16 Processes
				#define CARTESIAN_X 8
				...
				#define SETREDUCE 1 //reduce (1) or not reduce (0) ?
...
#define M 100			//total computation loop iterations.
...
//Initial setup for preperation of comm datatypes
void setup(int bigsizes[]);

//Set init values for our data matrix. uses rank for random seed
...
int main(int argc,char *argv[])  {
	...
	if (CARTESIAN_X*CARTESIAN_Y==numtasks)
	{
		...
		{	
			...
			/* better use (defined integers) UP DOWN LEFT & RIGHT than numbers. */
			...
			//setup the datatypes of the data segments to exchange.
			setup(bigsizes);
			
			/*here we get initial time before computations/communications part.*/
			T_init=MPI_Wtime();
			
			//commencing M iterations.
			for (i=0;i<M;i++)
			{
			  t1=MPI_Wtime();
			  ...
			  //Receiving the datatype data sections in reverse order
				...
				//dependent communication after receiving
				XY_plane_finite_dif_dep(x);
				physics_dep(z);
				...
				//waiting for send communications
			...
			MPI_Reduce (&sendbuf[comp],&Slowest_timer[comp],1,MPI_DOUBLE,MPI_MAX,0,myCartComm);
			MPI_Reduce (&sendbuf[comm],&Slowest_timer[comm],1,MPI_DOUBLE,MPI_MAX,0,myCartComm);
			MPI_Reduce (&sendbuf[overall],&Slowest_timer[overall],1,MPI_DOUBLE,MPI_MAX,0,myCartComm);
			if(SETREDUCE)
				MPI_Reduce (&sendbuf[fin_reduce],&Slowest_timer[fin_reduce],1,MPI_DOUBLE,MPI_MAX,0,myCartComm);
			
			if(!rank)//if rank=0, then print info
			{
				...
				printf("\n\t~----~\t\nCalculations maximum time:%.3lf ms\n\
				 				...
				 				1E3*Slowest_timer[comp],\
								1E3*Slowest_timer[comm],\
								1E3*Slowest_timer[overall]);
				if(SETREDUCE)
				{
					printf("\n# of Reductions = %d, T_reduction = %lf ms.\n",reductions,Slowest_timer[fin_reduce]*1E3);
					printf("\nTotal mass is: %.2e kg.\n",mass_reduced);
				}
				else
	...
	//wrong number of processors given?
	else 
	{
...
void setup(int bigsizes[])
{			
			/*setting up our datatypes to be exchanged.*/
			
			//send-to-nbors[LEFT]-datatype
			starts[0]=2;  starts[1]=2;  starts[2]=0;
			...
			//receive-from-nbors[LEFT]-datatype 	
			starts[0]=2;  starts[1]=0;  starts[2]=0;
			subsizes[0]=L-2-2;subsizes[1]=2;subsizes[2]=H;
			MPI_Type_create_subarray(count, bigsizes, subsizes, 
				starts,MPI_ORDER_C, MPI_DOUBLE, &LEFT_segment_RCV);
			MPI_Type_commit(&LEFT_segment_RCV);
			
			//send-to-nbors[RIGHT]-datatype 
			starts[0]=2;  starts[1]=W-4;  starts[2]=0;
			...
			//receive-from-nbors[RIGHT]datatype 	
			starts[0]=2;  starts[1]=W-2;  starts[2]=0;
			subsizes[0]=L-4;subsizes[1]=2;subsizes[2]=H;
			MPI_Type_create_subarray(count, bigsizes, subsizes, 
				starts,MPI_ORDER_C, MPI_DOUBLE, &RIGHT_segment_RCV);
			MPI_Type_commit(&RIGHT_segment_RCV);

			//send-to-nbors[UP]-datatype
			starts[0]=2;  starts[1]=2;  starts[2]=0;
			...
			//receive-from-nbors[UP]-datatype	
			starts[0]=0;  starts[1]=2;  starts[2]=0;
			subsizes[0]=2;subsizes[1]=W-4;subsizes[2]=H;
			MPI_Type_create_subarray(count, bigsizes, subsizes, 
				starts,MPI_ORDER_C, MPI_DOUBLE, &UP_segment_RCV);
			MPI_Type_commit(&UP_segment_RCV);

			//send-to-nbors[DOWN]-datatype
			starts[0]=L-4;  starts[1]=2;  starts[2]=0;
			subsizes[0]=2;subsizes[1]=W-4;subsizes[2]=H;
			MPI_Type_create_subarray(count, bigsizes, subsizes, 
				starts,MPI_ORDER_C, MPI_DOUBLE, &DOWN_segment_SEND);
			MPI_Type_commit(&DOWN_segment_SEND);

			//receive-from-nbors[DOWN]-datatype	
			starts[0]=L-2;  starts[1]=2;  starts[2]=0;
			subsizes[0]=2;subsizes[1]=W-4;subsizes[2]=H;
			MPI_Type_create_subarray(count, bigsizes, subsizes, 
